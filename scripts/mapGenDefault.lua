...
local function multioctave_noise(params)
  ...
  local octave_input_scale_multiplier = params.octave_input_scale_multiplier or 0.5
  ...
  if params.persistence then
    ...
    octave0_input_scale = octave0_input_scale * octave_input_scale_multiplier ^ (octave_count - 1)
    -- 'persistence' implies that the octaves would otherwise have been powers of 2, I think
    octave0_output_scale = octave0_output_scale * 2 ^ (octave_count - 1)
...
local standard_starting_lake_elevation_expression = noise.define_noise_function( function(x,y,tile,map)
  ...
  local lake = noise.ident(noise.min(
...
local function simple_amplitude_corrected_multioctave_noise(params)
  ...
  return simple_variable_persistence_multioctave_noise(util2.merge{params, {octave0_output_scale = multiplier * amplitide}})
end
...
local function default_world_elevation(x,y,tile,map)
...
return {
  default_world_elevation = default_world_elevation,
